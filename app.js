...
function aiVoiceCue(kind='wake'){try{const C=window.AudioContext||window.webkitAudioContext;if(!C)return;const c=new C(),g=c.createGain(),o=c.createOscillator();o.type='sine';o.frequency.setValueAtTime(kind==='wake'?880:520,c.currentTime);o.frequency.exponentialRampToValueAtTime(kind==='wake'?1320:660,c.currentTime+.11);g.gain.setValueAtTime(.0001,c.currentTime);g.gain.exponentialRampToValueAtTime(.18,c.currentTime+.015);g.gain.exponentialRampToValueAtTime(.0001,c.currentTime+.18);o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+.2);o.onended=()=>c.close().catch(()=>{})}catch{}}
