...
function aiVoiceCue(kind='wake'){
  try{
    ...
    const c=new C(),master=c.createGain();master.gain.value=.72;master.connect(c.destination);
    const play=(freq,at,duration=.16)=>{const o=c.createOscillator(),g=c.createGain();o.type='sine';o.frequency.setValueAtTime(freq,at);g.gain.setValueAtTime(.0001,at);g.gain.exponentialRampToValueAtTime(.62,at+.012);g.gain.exponentialRampToValueAtTime(.0001,at+duration);o.connect(g);g.connect(master);o.start(at);o.stop(at+duration+.02)};
    const now=c.currentTime+.025;
    if(kind==='wake'){play(880,now,.17);play(1320,now+.14,.22)}else{play(660,now,.2)}
    setTimeout(()=>c.close().catch(()=>{}),650);
  }catch{}
}
