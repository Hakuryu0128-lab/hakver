/* ═══════════════════════════════════════════════════════════
   ...
   title属性でホバー確認できる）。
   v11.17.0：デザイン統一Phase2で追加した週案（#view-weekly）のグラデー
   ション背景を撤回。ユーザー指摘：授業が入っていないマスは透明で
   グラデーションが透けるが、授業が入ったマス（.grid-cell.has-lesson）は
   不透明background: var(--bg)のままなので、実際に授業を登録していくと
   グラデーションがマスの形にくり抜かれたように見えて見づらい・見栄えが
   悪い。週案は他の一覧ページと違いグリッドがほぼ隙間なくマスで埋まる
   情報密度の高い画面なので、透明・半透明を活かすガラス表現よりも
   読みやすさを優先し、グラデーション無しの不透明背景に戻した（進度・
   月行事・写真・出席・評価・バックアップ・ToDo・印刷・名簿・設定・メモは
   引き続きグラデーションのまま。これらは授業マスのような細かい不透明
...
/* ══════════════════════════════════════════════════════════════
   Hauryuver: AIチャット（サイドバー・Gemini API・tool calling）
   (Decisions: 2026-08-18-weeky-ai-sidebar-gemini-integration)
   - ローカルLLMはブラウザで動かせないため、Gemini APIをfetchで直接叩く。
   - ToDoの読み書きはfunction callingでapp.js側の関数に委譲する。
   - state.aiChat.messages は表示用の簡略ログ（role: user/ai/system/error, text）。
     APIへ送るcontentsは毎回この簡略ログから組み立て直す（tool呼び出しの
     中間状態は会話をまたいで保持しない＝ツールは呼べば毎回最新状態を返すので実害なし）。
   ══════════════════════════════════════════════════════════════ */

const AI_MAX_TOOL_LOOPS = 6;

function aiClassCatalogue(){const schools=(state.schools||[]).map(sc=>{const m=new Map(),add=(raw,src,meta={})=>{let name=normClass(raw),pre=normClass(sc.name);if(pre&&name.startsWith(pre))name=name.slice(pre.length).trim();const p=parseClassText(name);if(!p)return;const cn=makeClassName(p.grade,p.classNo),k=normClass(cn);if(!m.has(k))m.set(k,{className:cn,lessonClassName:effectiveClassName(cn,sc.id),students:new Set(),sources:new Set()});const x=m.get(k);if(meta.studentId)x.students.add(String(meta.studentId));x.sources.add(src)};(state.classes||[]).filter(c=>c.schoolId===sc.id).forEach(c=>add(c.name,'classes',c));(state.students||[]).filter(s=>s.schoolId===sc.id).forEach(s=>add(s.className||deriveClassName(s.grade,s.classNo),'students',{studentId:s.id||s.qrId}));Object.values(state.lessons||{}).forEach(l=>{const r=normClass(l?.className);if(r&&normClass(sc.name)&&r.startsWith(normClass(sc.name)))add(r,'lessons')});return{schoolId:sc.id,schoolName:sc.name,isActive:sc.id===state.activeSchoolId,classes:[...m.values()].map(x=>({className:x.className,lessonClassName:x.lessonClassName,studentCount:x.students.size,sources:[...x.sources]})).sort((a,b)=>a.className.localeCompare(b.className,'ja',{numeric:true}))}});return{ok:true,activeSchoolId:state.activeSchoolId||'',schools}}
...
function aiToolDeclarations() {
  return [{
    functionDeclarations: [
      {
        name: 'add_todo',
        description: 'WEEKYのToDoリストに新しい項目を追加する。',
        parameters: {
          type: 'object',
          properties: {
            text: { type: 'string', description: 'ToDoの内容' },
            due:  { type: 'string', description: '締切日（YYYY-MM-DD形式、任意）' },
            tags: { type: 'array', items: { type: 'string' }, description: 'タグ（任意・複数可）' },
          },
          required: ['text'],
        },
      },
      {
        name: 'list_todos',
        description: '現在のToDo一覧を取得する（id・内容・完了状態・締切・タグ）。完了/未完了の両方を返す。',
        parameters: { type: 'object', properties: {} },
      },
      {
        name: 'toggle_todo',
        description: '指定したToDoの完了状態を反転する。事前にlist_todosでidを確認すること。',
        parameters: {
          type: 'object',
          properties: { id: { type: 'string', description: 'ToDoのid' } },
          required: ['id'],
        },
      },
      {
        name: 'update_todo',
        description: '指定したToDoの内容・締切・タグを変更する。渡したフィールドだけ更新される。',
        parameters: {
          type: 'object',
          properties: {
            id:   { type: 'string', description: 'ToDoのid' },
            text: { type: 'string', description: '新しい内容（任意）' },
            due:  { type: 'string', description: '新しい締切日 YYYY-MM-DD（任意・空文字で解除）' },
      ...
      {
        name: 'delete_todo',
        description: '指定したToDoを削除する。事前にlist_todosでidを確認すること。',
        parameters: {
          type: 'object',
          properties: { id: { type: 'string', description: 'ToDoのid' } },
          required: ['id'],
...
/* ツール名 → 実行後にチャットへ出す短い日本語ラベル（透明性のため） */
...
function aiExecuteTool(name,args){if((_aiAttachmentAnalysisMode||_aiUrlContextForRequest)&&['add_todo','set_lesson','add_note','update_todo','delete_todo','delete_lesson','toggle_todo','update_note','delete_note'].includes(name))return{error:'confirmation_required'};
  args = args || {};
  try {
    switch (name) {
      case 'add_todo': {
        ...
        const todo = {
          id: uid(), text: String(args.text).trim(), done: false,
          due: args.due || '', tags: Array.isArray(args.tags) ? args.tags.filter(Boolean) : [],
          createdAt: Date.now(),
        };
      ...
      case 'list_todos': {
        return {
          ...
          todos: state.todos.map(t => ({
            id: t.id, text: t.text, done: !!t.done, due: t.due || '', tags: t.tags || [],
          })),
        };
      }
      case 'toggle_todo': {
        const t = state.todos.find(x => x.id === args.id);
        if (!t) return { error: 'todo not found', id: args.id };
        t.done = !t.done;
        t.doneAt = t.done ? Date.now() : null;
        save();
        _aiRefreshTodoUi();
        return { ok: true, todo: { id: t.id, text: t.text, done: t.done } };
      }
      case 'update_todo': {
        const t = state.todos.find(x => x.id === args.id);
        if (!t) return { error: 'todo not found', id: args.id };
        if (typeof args.text === 'string' && args.text.trim()) t.text = args.text.trim();
        if (typeof args.due === 'string') t.due = args.due;
        if (Array.isArray(args.tags)) t.tags = args.tags.filter(Boolean);
        save();
        _aiRefreshTodoUi();
        return { ok: true, todo: { id: t.id, text: t.text, due: t.due, tags: t.tags } };
      }
      case 'delete_todo': {
        const idx = state.todos.findIndex(x => x.id === args.id);
        if (idx < 0) return { error: 'todo not found', id: args.id };
      ...
      case 'set_lesson': {
        ...
        if(args.className!==undefined&&nextClassName){const valid=aiClassCatalogue().schools.flatMap(s=>s.classes.map(c=>c.lessonClassName));const hit=valid.find(c=>normClass(c)===normClass(nextClassName));if(!hit)return{error:'unknown className'};nextClassName=hit;}
        state.lessons[key] = {
          ...prev,
...
function aiBuildContentsFromHistory() {
...
function aiExtractUrls(text){const a=String(text||'').match(/https?:\/\/[^\s<>"')\]}]+/gi)||[];return [...new Set(a.map(u=>u.replace(/[.,;:!?、。）」』】]+$/g,'')))].slice(0,5)}
async function aiCallGemini(contents, _retry = 0) {
  ...
  const body={contents,tools:_aiUrlContextForRequest?[{urlContext:{}},...aiToolDeclarations()]:aiToolDeclarations(),systemInstruction:{parts:[{text:aiSystemInstructionText()+((_aiUrlContextForRequest||_aiAttachmentAnalysisMode)?'\n\n【資料の初回解析】内容を要約し、日付・締切・予定・担当・提出物を抽出して、ToDo・週案・メモ候補をMarkdownで提示してください。音声なら文字起こしと決定事項も整理してください。初回は変更ツールを使わず、最後に「登録して、と言われたら実行します」と伝えてください。':'\n\n【通常時】Web検索は使わず、WEEKY内部ツールと会話内の情報だけで答えてください。')}]}};
...
function aiPushMessage(role, text) {
  state.aiChat.messages.push({ role, text, at: Date.now() });
  // 会話ログが際限なく伸びないよう直近80件に制限
  if (state.aiChat.messages.length > 80) state.aiChat.messages.splice(0, state.aiChat.messages.length - 80);
  save();
}

let _aiPendingFile=null,_aiAttachmentAnalysisMode=false,_aiVoiceReplyPending=false;
const AI_INLINE_MAX_BYTES=19*1024*1024,AI_TEXT_MAX_CHARS=300000;
function aiClearPendingFile(){_aiPendingFile=null;const i=document.getElementById('aiChatImageInput'),p=document.getElementById('aiAttachmentPreview'),t=document.getElementById('aiAttachmentThumb');if(i)i.value='';if(p)p.hidden=true;if(t){t.hidden=true;t.removeAttribute('src')}}
...
let _aiWakeEnabled=false,_aiWakeRecognition=null,_aiWakeRestartTimer=null,_aiWakeSuppressClick=false,_aiWakeLastHit=0;
const AI_WAKE_WORDS=['ねえweeky','ねぇweeky','ねえwiki','ねぇwiki','heyweeky','weeky','wiki','ウィーキー','ウィーキ','ウィキ','ウイーキー','ウィッキー','ウェイキー','ウェイキ'];
function aiNormalizeWakeText(s){return String(s||'').toLowerCase().replace(/[\s、。,.!！?？ーｰ～~・「」『』]/g,'').replace(/ｗｅｅｋｙ/g,'weeky').replace(/ｗｉｋｉ/g,'wiki')}
function aiWakeMatched(raw){const t=aiNormalizeWakeText(raw);if(!t)return false;if(AI_WAKE_WORDS.some(w=>t.includes(aiNormalizeWakeText(w))))return true;return /ね[えぇ]?(?:う?[ぃい]?き|う?[ぇえ]い?き|wiki|weeky)/.test(t)}
...
function aiVoiceCue(kind='wake'){
  try{
    const C=window.AudioContext||window.webkitAudioContext;if(!C)return;
    const c=new C(),master=c.createGain();master.gain.value=.82;master.connect(c.destination);
    const play=(freq,at,duration=.16,type='sine')=>{const o=c.createOscillator(),g=c.createGain();o.type=type;o.frequency.setValueAtTime(freq,at);g.gain.setValueAtTime(.0001,at);g.gain.exponentialRampToValueAtTime(.7,at+.012);g.gain.exponentialRampToValueAtTime(.0001,at+duration);o.connect(g);g.connect(master);o.start(at);o.stop(at+duration+.02)};
    const now=c.currentTime+.025;
    if(kind==='fail'){play(440,now,.17,'triangle');play(220,now+.15,.28,'triangle')}
    else if(kind==='wake'){play(880,now,.16);play(1320,now+.14,.22)}
    else {play(760,now,.22)}
    setTimeout(()=>c.close().catch(()=>{}),750);
  }catch{}
}
...
function aiStartWakeRecognition(){if(!_aiWakeEnabled||_aiChatBusy||_aiRecording||_aiFastRecognition||window.speechSynthesis?.speaking)return;if(!aiWakeSupported()){aiDisableWakeMode('この端末ではウェイクワード待機に対応していません');return}aiStopWakeRecognition();const R=window.SpeechRecognition||window.webkitSpeechRecognition,r=new R();_aiWakeRecognition=r;r.lang='ja-JP';r.continuous=true;r.interimResults=true;r.maxAlternatives=5;r.onresult=e=>{for(let i=e.resultIndex;i<e.results.length;i++){const result=e.results[i];for(let a=0;a<Math.min(result.length,5);a++){if(aiWakeMatched(result[a]?.transcript||'')&&Date.now()-_aiWakeLastHit>2500){_aiWakeLastHit=Date.now();r.onend=null;try{r.stop()}catch{} _aiWakeRecognition=null;aiVoiceCue('wake');aiVoiceStatus('はい、どうぞ。2音鳴ったあとに話してください');setTimeout(()=>aiStartFastVoiceRecognition(true,false),700);return}}}};r.onerror=e=>{if(['not-allowed','service-not-allowed'].includes(e.error)){aiDisableWakeMode('マイクまたは音声認識の許可が必要です')}};r.onend=()=>{_aiWakeRecognition=null;if(_aiWakeEnabled)_aiWakeRestartTimer=setTimeout(aiStartWakeRecognition,450)};try{r.start();aiSetWakeUi(true);aiVoiceStatus('「ねえWEEKY」と話しかけてください')}catch{_aiWakeRestartTimer=setTimeout(aiStartWakeRecognition,700)}}
async function aiToggleWakeMode(){if(_aiWakeEnabled){aiDisableWakeMode();return}if(!aiWakeSupported()){aiVoiceStatus('この端末ではウェイクワード待機に対応していません。通常のマイク入力は使えます。',true);return}try{const s=await navigator.mediaDevices.getUserMedia({audio:true});s.getTracks().forEach(t=>t.stop());_aiWakeEnabled=true;aiSetWakeUi(true);aiVoiceCue('ready');aiStartWakeRecognition()}catch{aiVoiceStatus('マイクの使用を許可してください',true)}}
...
function aiBrokenTranscript(text){const s=String(text||'').replace(/\s/g,'').trim();if(!s||s==='__NO_SPEECH__')return true;if(s.length>60&&/^\d+$/.test(s))return true;for(let n=1;n<=10;n++){if(s.length>=n*7){const u=s.slice(0,n);if(u.repeat(Math.ceil(s.length/n)).slice(0,s.length)===s)return true}}return false}
function aiApiAudioMime(blob){const raw=String(blob?.type||'').toLowerCase().split(';')[0].trim();if(raw==='audio/mp4'||raw==='audio/mpeg'||raw==='audio/wav'||raw==='audio/ogg'||raw==='audio/webm'||raw==='audio/aac'||raw==='audio/flac')return raw;return 'audio/webm'}
async function aiToggleVoiceInput(){if(_aiChatBusy)return;if(_aiFastRecognition){aiStopFastVoiceRecognition();return}aiStopWakeRecognition();aiVoiceCue('ready');aiVoiceStatus('音のあとに話してください');setTimeout(()=>aiStartFastVoiceRecognition(false,true),360)}
let _aiFastRecognition=null,_aiFastTimer=null,_aiFastFinalized=false;
function aiStopFastVoiceRecognition(){clearTimeout(_aiFastTimer);_aiFastTimer=null;if(_aiFastRecognition){const r=_aiFastRecognition;_aiFastRecognition=null;r.onend=null;try{r.abort()}catch{}}document.getElementById('aiChatMicBtn')?.classList.remove('is-recording')}
function aiFastVoiceFail(message='音声を認識できませんでした'){
  aiStopFastVoiceRecognition();aiVoiceCue('fail');aiVoiceStatus(message,true);
  if(_aiWakeEnabled)setTimeout(aiStartWakeRecognition,1000);
}
function aiStartFastVoiceRecognition(fromWake=false,manual=false){
  if(_aiChatBusy)return;
  if(!aiWakeSupported()){aiFastVoiceFail('この端末では高速音声認識に対応していません');return}
  aiStopFastVoiceRecognition();window.speechSynthesis?.cancel();
  const R=window.SpeechRecognition||window.webkitSpeechRecognition,r=new R();_aiFastRecognition=r;_aiFastFinalized=false;
  r.lang='ja-JP';r.continuous=false;r.interimResults=true;r.maxAlternatives=3;
  let finalText='',interimText='',gotSpeech=false;
  const input=document.getElementById('aiChatInput'),mic=document.getElementById('aiChatMicBtn');mic?.classList.add('is-recording');aiVoiceStatus('聞いています…');
  const finish=text=>{text=String(text||'').trim();if(_aiFastFinalized)return;_aiFastFinalized=true;aiStopFastVoiceRecognition();if(!text||aiBrokenTranscript(text)){aiFastVoiceFail();return}if(input){input.value=text;input.style.height='auto';input.style.height=Math.min(90,input.scrollHeight)+'px'}aiVoiceStatus(`認識: ${text}`);_aiVoiceReplyPending=true;setTimeout(()=>{if(input){input.value='';input.style.height='auto'}sendAiChatMessage(text)},80)};
  r.onresult=e=>{for(let i=e.resultIndex;i<e.results.length;i++){const result=e.results[i],text=result[0]?.transcript||'';if(text.trim())gotSpeech=true;if(result.isFinal)finalText+=(finalText?' ':'')+text;else interimText=text}const shown=(finalText||interimText).trim();if(input&&shown){input.value=shown;input.style.height='auto';input.style.height=Math.min(90,input.scrollHeight)+'px'}if(finalText.trim())finish(finalText)};
  r.onspeechend=()=>{try{r.stop()}catch{}};
  r.onerror=e=>{if(_aiFastFinalized)return;const denied=['not-allowed','service-not-allowed'].includes(e.error);aiFastVoiceFail(denied?'マイクまたは音声認識の許可が必要です':'音声を認識できませんでした')};
  r.onend=()=>{if(_aiFastFinalized)return;const text=(finalText||interimText).trim();if(text)finish(text);else aiFastVoiceFail(gotSpeech?'音声を文字にできませんでした':'音声を認識できませんでした')};
  _aiFastTimer=setTimeout(()=>{if(!_aiFastFinalized){try{r.stop()}catch{} }},9000);
  try{r.start()}catch{aiFastVoiceFail('音声認識を開始できませんでした')}
}

async function aiStartVoiceRecording(autoStop=false){if(_aiRecording||_aiChatBusy)return;try{window.speechSynthesis?.cancel();_aiMicStream=await navigator.mediaDevices.getUserMedia({audio:{echoCancellation:true,noiseSuppression:true,autoGainControl:true,channelCount:1}});let mime='';for(const x of ['audio/webm;codecs=opus','audio/webm','audio/mp4'])if(window.MediaRecorder?.isTypeSupported?.(x)){mime=x;break}_aiAudioChunks=[];_aiRecorder=new MediaRecorder(_aiMicStream,mime?{mimeType:mime}:undefined);let audioCtx=null,analyser=null,maxTimer=null,startTimer=null,raf=0,heardVoice=false,lastVoice=performance.now();const cleanup=()=>{cancelAnimationFrame(raf);clearTimeout(maxTimer);clearTimeout(startTimer);try{audioCtx?.close()}catch{}};if(autoStop&&(window.AudioContext||window.webkitAudioContext)){const C=window.AudioContext||window.webkitAudioContext;audioCtx=new C();const src=audioCtx.createMediaStreamSource(_aiMicStream);analyser=audioCtx.createAnalyser();analyser.fftSize=1024;analyser.smoothingTimeConstant=.35;src.connect(analyser);const bins=new Uint8Array(analyser.fftSize);const watch=()=>{if(!_aiRecording)return;analyser.getByteTimeDomainData(bins);let sum=0;for(const v of bins){const d=(v-128)/128;sum+=d*d}const rms=Math.sqrt(sum/bins.length);if(rms>.009){heardVoice=true;lastVoice=performance.now()}if(heardVoice&&performance.now()-lastVoice>1700){_aiRecorder?.stop();return}raf=requestAnimationFrame(watch)};setTimeout(watch,80);startTimer=setTimeout(()=>{if(_aiRecording&&!heardVoice)_aiRecorder?.stop()},7500);maxTimer=setTimeout(()=>{if(_aiRecording)_aiRecorder?.stop()},25000)}_aiRecorder.ondataavailable=e=>{if(e.data?.size)_aiAudioChunks.push(e.data)};_aiRecorder.onerror=()=>aiVoiceStatus('録音中にエラーが発生しました',true);_aiRecorder.onstop=async()=>{cleanup();_aiRecording=false;document.getElementById('aiChatMicBtn')?.classList.remove('is-recording');_aiMicStream?.getTracks().forEach(t=>t.stop());if(autoStop&&!heardVoice){aiVoiceStatus('声を認識できませんでした。もう一度「ねえWEEKY」と呼んでください',true);if(_aiWakeEnabled)setTimeout(aiStartWakeRecognition,700);return}const blob=new Blob(_aiAudioChunks,{type:aiApiAudioMime({type:_aiRecorder.mimeType})});if(blob.size<1400){aiVoiceStatus('音声を認識できませんでした',true);if(_aiWakeEnabled)setTimeout(aiStartWakeRecognition,700);return}if(blob.size>AI_INLINE_MAX_BYTES){aiVoiceStatus('録音が長すぎます。短く区切ってください',true);if(_aiWakeEnabled)setTimeout(aiStartWakeRecognition,700);return}aiVoiceStatus('音声を文字起こししています…');try{const transcript=await aiTranscribeVoice(blob);if(aiBrokenTranscript(transcript))throw new Error('音声を正しく認識できませんでした。もう一度話してください');const input=document.getElementById('aiChatInput');if(input){input.value=transcript;input.style.height='auto'}aiVoiceStatus(`認識: ${transcript}`);_aiVoiceReplyPending=true;setTimeout(()=>{const box=document.getElementById('aiChatInput');if(box){box.value='';box.style.height='auto'}sendAiChatMessage(transcript)},120)}catch(err){aiVoiceStatus(err?.message||'音声認識に失敗しました',true);if(_aiWakeEnabled)setTimeout(aiStartWakeRecognition,900)}};_aiRecorder.start(250);_aiRecording=true;document.getElementById('aiChatMicBtn')?.classList.add('is-recording');aiVoiceStatus(autoStop?'話してください。話し終わると自動で送信します':'録音中です。話し終わったらマイクを押してください')}catch(err){aiVoiceStatus(err?.name==='NotAllowedError'?'マイクの使用を許可してください':'マイクを開始できませんでした',true);if(_aiWakeEnabled)setTimeout(aiStartWakeRecognition,700)}}
async function aiTranscribeVoice(blob){const apiKey=state.settings.ai?.apiKey?.trim(),configured=state.settings.ai?.model?.trim()||'gemini-3.5-flash-lite';if(!apiKey)throw new Error('APIキーが未設定です');const mimeType=aiApiAudioMime(blob),data=await aiFileToBase64(new File([blob],mimeType==='audio/mp4'?'voice.m4a':'voice.webm',{type:mimeType}));const models=[configured,'gemini-2.5-flash'].filter((x,i,a)=>x&&a.indexOf(x)===i);let last='';for(const model of models){const url=`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`,body={contents:[{role:'user',parts:[{text:'この音声の発話内容だけを日本語で正確に文字起こししてください。説明、引用符、前置き、Markdownは不要です。無音、ノイズのみ、音声破損、または判別不能なら __NO_SPEECH__ だけを返してください。同じ数字や同じ語を補完して繰り返さないでください。'},{inlineData:{mimeType,data}}]}]};const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),json=await res.json().catch(()=>null);if(res.ok)return(json?.candidates?.[0]?.content?.parts||[]).map(p=>p.text||'').join('').trim();last=json?.error?.message||`HTTP ${res.status}`;if(!/invalid argument|not found|unsupported/i.test(last))break}throw new Error(`音声認識エラー: ${last}`)}

async function sendAiChatMessage(userText) {
  userText=(userText||'').trim();const attachedFile=_aiPendingFile;if((!userText&&!attachedFile)||_aiChatBusy)return;if(!userText&&attachedFile)userText=attachedFile.category==='audio'?'この録音を文字起こしして要約し、決定事項・期限・担当・ToDo候補を整理して。':'このファイルを読み取り、要点と登録候補を整理して。';const urls=aiExtractUrls(userText);_aiUrlContextForRequest=urls.length>0;if(_aiUrlContextForRequest&&urls.length===1&&userText===urls[0])userText=`次のURLの内容を読み取り、要点と登録候補を整理して。\n${urls[0]}`;
...
function _aiSetChatBusyUi(busy) {
  ...
  const mic = document.getElementById('aiChatMicBtn');
...
function aiMarkdownHtml(v){const e=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),i=s=>e(s).replace(/\*\*([^*\n]+)\*\*/g,'<strong>$1</strong>').replace(/`([^`\n]+)`/g,'<code>$1</code>');let o=[],l=null;for(const x of String(v||'').replace(/<br\s*\/?>/gi,'\n').split(/\r?\n/)){let m;if(!x.trim()){l=null;continue}if((m=x.match(/^(#{1,4})\s+(.+)$/))){o.push(`<h${m[1].length}>${i(m[2])}</h${m[1].length}>`);continue}if((m=x.match(/^\s*[-*+]\s+(.+)$/))){if(!l){l=[];o.push(l)}l.push(m[1]);continue}if(Array.isArray(o[o.length-1]))continue;o.push(`<p>${i(x)}</p>`)}return o.map(x=>Array.isArray(x)?`<ul>${x.map(y=>`<li>${i(y)}</li>`).join('')}</ul>`:x).join('')}
...
function renderAiChat() {
  ...
  box.querySelectorAll('.ai-chat-msg').forEach(el => el.remove());
  if (empty) empty.style.display = msgs.length ? 'none' : 'block';
  msgs.forEach(m => {
    const div = document.createElement('div');
    div.className = 'ai-chat-msg ' + (AI_ROLE_CLASS[m.role] || 'ai-chat-msg--ai');
    if(m.role==='ai')div.innerHTML=aiMarkdownHtml(m.text);else div.textContent=m.text;
    box.appendChild(div);
  });
  box.scrollTop = box.scrollHeight;
}
...
const SETTINGS_SAVE_CATS = ['account', 'lesson', 'ai'];
...
async function startApp() {
  ...
  updatePanelTodo();
  updateClassStatus();
  renderAiChat(); // Hauryuver: サイドバーAIチャットの会話履歴を復元表示
  updateStats();
  loadWeather();
...
function bindEvents() {
  ...
  q('aiChatInput')?.addEventListener('keydown', e => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      q('aiChatForm')?.requestSubmit();
    }
  });
  q('aiChatInput')?.addEventListener('input', e => {
    e.target.style.height = 'auto';
    e.target.style.height = Math.min(90, e.target.scrollHeight) + 'px';
  });
  q('aiChatClearBtn')?.addEventListener('click', async () => {
    if (!state.aiChat?.messages?.length) return;
    if (await customConfirm('AIとの会話をクリアしますか?')) clearAiChat();
  });
